...
<task type="auto">
  <name>Task 1: Create drawn cursor module</name>
...
4. **create_cursor(index: usize, config: DrawnCursorConfig) -> DrawnCursor**:
   ...
   - Set cursor arrays: cursor_char, cursor_alt_char, cursor_style, cursor_blink_fps
   - Create manual_visible signal
   - Register focus callbacks:
     - on_focus: subscribe to blink if enabled
     - on_blur: unsubscribe from blink
   - Set cursor_visible getter that checks:
     1. Manual override (if Some)
     2. If not focused, return true (cursor shows but doesn't blink)
     3. If focused but blink disabled, return true
...
Update `src/state/mod.rs` to export drawn_cursor module.
  ...
  - cursor_visible getter respects focus and blink state
  </done>
</task>
...
- [ ] cursor_visible getter integrates blink phase correctly
