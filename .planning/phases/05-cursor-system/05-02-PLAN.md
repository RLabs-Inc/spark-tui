...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/engine/arrays/interaction.rs"
      provides: "cursor_char, cursor_alt_char, cursor_style arrays with accessors"
...
Purpose: Provide a clean API for terminal cursor control and add cursor_char/cursor_alt_char/cursor_style arrays needed for drawn cursor configuration.
...
<task type="auto">
  <name>Task 2: Add cursor character arrays with full accessor API</name>
...
**CRITICAL: Add ALL accessor functions (Plan 05-03 depends on these):**
...
These accessors are essential for Plan 05-03's drawn cursor module to configure cursor appearance per-component.
  </action>
  <verify>
  ```bash
  cargo build -p spark-tui 2>&1 | head -30
  cargo test -p spark-tui interaction --no-fail-fast 2>&1 | tail -30
  # Verify all accessors exist
  grep -E "pub fn (get|set)_cursor_(char|alt_char|style)" src/engine/arrays/interaction.rs
  ```
  </verify>
  <done>
  - CURSOR_CHAR, CURSOR_ALT_CHAR, CURSOR_STYLE arrays exist
  - All 6 accessor functions exported: get/set_cursor_char, get/set_cursor_alt_char, get/set_cursor_style
...
# Verify all accessors exist for downstream plans
grep -E "pub fn (get|set)_cursor_(char|alt_char|style)" src/engine/arrays/interaction.rs | wc -l
# Should output: 6
```
</verification>
...
- [ ] All 6 accessor functions for cursor arrays are exported
- [ ] All tests pass
