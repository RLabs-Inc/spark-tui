   ...
   - Inside `scope.run()`:
     - Create effect that:
       ...
       d. For each item:
          ...
          - If duplicate key detected, warn via eprintln and skip (no crash)
...
5. **Duplicate key handling:** Use `eprintln!` to warn about duplicate keys without crashing. This matches TypeScript's console.warn behavior.

6. **ReactiveVec support:** The `items_getter: impl Fn() -> Vec<T>` pattern supports ReactiveVec via the getter. Users can pass `move || reactive_vec.to_vec()` to get reactive list updates. This is the idiomatic Rust approach - no overload needed.

7. **Update `src/primitives/mod.rs` export** (if not already done in plan 01):
   - Ensure `pub use control_flow::each;` is present
  </action>
  <verify>
...
<task type="auto">
  ...
  <files>src/primitives/control_flow.rs</files>
  <action>
**Pre-verification:** Confirm registry API availability (same as plan 01):
...
9. **test_each_duplicate_key_no_crash:**
   - Create items with duplicate keys (same id)
   - Verify no crash occurs
   - (Warning is printed to stderr but we don't verify stderr output)
...
- each() handles duplicate keys without crash (warning only)
