...
2. **WhenOptions struct:**
   ```rust
   /// Options for when() async rendering.
   ///
   /// At minimum, `then_fn` is required. `pending_fn` and `catch_fn` are optional.
       ...
       E: Clone + std::fmt::Display + 'static,
   {
       ...
       // PhantomData for T and E
       _marker: std::marker::PhantomData<(T, E)>,
   }
   ```

3. **WhenOptions constructor:**
   ```rust
   impl<T, E, PendingF, ThenF, CatchF> WhenOptions<T, E, PendingF, ThenF, CatchF>
   where
       T: Clone + 'static,
       E: Clone + std::fmt::Display + 'static,
   {
       /// Create WhenOptions with required then_fn.
       pub fn new(then_fn: ThenF) -> Self
       where
           PendingF: Default,
           CatchF: Default,
       {
...
4. **Update exports in `src/primitives/mod.rs`:**
   - Add `pub use control_flow::{when, AsyncState, WhenOptions};`
  </action>
  <verify>
...
4. **Note on type inference:** The WhenOptions struct with PhantomData may cause inference issues. Consider a builder pattern or helper function if needed:
   ```rust
   pub fn when_options<T, E>() -> WhenOptionsBuilder<T, E> { ... }
   ```
   But start simple - direct struct construction should work.
  </action>
  <verify>
`cargo check -p spark-tui` compiles without errors.
  </verify>
...
- WhenOptions struct exported with pending/then_fn/catch_fn fields
