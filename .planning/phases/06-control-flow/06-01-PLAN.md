...
6. **Update `src/primitives/mod.rs`:**
   ...
   - Add `pub use control_flow::{show, each, when, AsyncState, WhenOptions};`

   **Note:** Export all three functions initially (each, when will be implemented in later plans). Use `#[allow(unused_imports)]` temporarily if needed, or add stub declarations that will be filled in by plans 02 and 03.
...
Module exports show from primitives (with placeholders for each, when if needed).
  </done>
</task>
...
<task type="auto">
  ...
  <action>
**Pre-verification:** Confirm registry API availability by checking `src/engine/registry.rs` exports:
- `reset_registry` - resets all state (for test isolation)
- `allocate_index(id: Option<&str>) -> usize` - allocates component index
- `release_index(index: usize)` - releases component index
- `get_allocated_count() -> usize` - returns count of allocated indices
- `push_parent_context(index: usize)` / `pop_parent_context()` - parent stack
- `get_current_parent_index() -> Option<usize>` - current parent
