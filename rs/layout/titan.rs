//! TITAN Layout Engine (legacy - replaced by Taffy)
//!
...
pub fn compute_layout(
    ...
    for &i in bfs_queue.iter().rev() {
        ...
        if comp_type == ComponentType::Text {
                    ...
                    INTRINSIC_H.with(|h| {
                        ...
                        let avail_w = if let Some(p) = parent {
                            if let Some(node) = get_flex_node(p) {
                                let parent_w = resolve_dimension(node.width.get().unwrap_or(Dimension::Auto), terminal_width);
                                let pad_l = node.padding_left.get().unwrap_or(0);
                                let pad_r = node.padding_right.get().unwrap_or(0);
                                let bord_l = if node.border_left.get().unwrap_or(0) > 0 { 1 } else { 0 };
                                let bord_r = if node.border_right.get().unwrap_or(0) > 0 { 1 } else { 0 };
                                if parent_w > 0 {
            ...
            if let Some(node) = get_flex_node(i) {
                ...
                let bord_l = if node.border_left.get().unwrap_or(0) > 0 { 1 } else { 0 };
                let bord_r = if node.border_right.get().unwrap_or(0) > 0 { 1 } else { 0 };
                let bord_t = if node.border_top.get().unwrap_or(0) > 0 { 1 } else { 0 };
                let bord_b = if node.border_bottom.get().unwrap_or(0) > 0 { 1 } else { 0 };
                        ...
                        INTRINSIC_W.with(|intr_w| {
                            INTRINSIC_H.with(|intr_h| {
                                ...
                                if kid != -1 && !is_scrollable {
                                    let dir = node.flex_direction.get().unwrap_or(0);
                                    let is_row = dir == FLEX_ROW || dir == FLEX_ROW_REVERSE;
                                    let gap = node.gap.get().unwrap_or(0);
                                    ...
                                    while kid >= 0 {
                                        ...
                                        if let Some(kid_node) = get_flex_node(k) {
                                            let explicit_w = resolve_dimension(kid_node.width.get().unwrap_or(Dimension::Auto), terminal_width);
                                            let explicit_h = resolve_dimension(kid_node.height.get().unwrap_or(Dimension::Auto), terminal_height);
                                        ...
                                        let margin_main = if let Some(kid_node) = get_flex_node(k) {
                                            if is_row {
                                                kid_node.margin_left.get().unwrap_or(0) + kid_node.margin_right.get().unwrap_or(0)
                                            } else {
                                                kid_node.margin_top.get().unwrap_or(0) + kid_node.margin_bottom.get().unwrap_or(0)
                                            }
                                        } else {
                                            0
                                        };
    ...
    for &root in bfs_queue.iter().take(root_count) {
        if let Some(node) = get_flex_node(root) {
            let ew = resolve_dimension(node.width.get().unwrap_or(Dimension::Auto), terminal_width);
            let eh = resolve_dimension(node.height.get().unwrap_or(Dimension::Auto), terminal_height);
    ...
    for &i in &indices {
        if let Some(node) = get_flex_node(i) {
            if node.position.get().unwrap_or(POS_RELATIVE) == POS_ABSOLUTE {
                layout_absolute(
                    i,
                    &indices,
                    terminal_width,
...
fn layout_children(
    ...
    FIRST_CHILD.with(|first| {
        NEXT_SIBLING.with(|next| {
            ...
            while kid >= 0 {
                let k = kid as usize;
                if let Some(kid_node) = get_flex_node(k) {
                    if kid_node.position.get().unwrap_or(POS_RELATIVE) != POS_ABSOLUTE {
                        flow_kids.push(k);
                    }
                } else {
                    flow_kids.push(k);
    ...
    let p_pad_t = parent_node.padding_top.get().unwrap_or(0);
    let p_pad_r = parent_node.padding_right.get().unwrap_or(0);
    let p_pad_b = parent_node.padding_bottom.get().unwrap_or(0);
    let p_pad_l = parent_node.padding_left.get().unwrap_or(0);
    let p_bord_t = if parent_node.border_top.get().unwrap_or(0) > 0 { 1 } else { 0 };
    let p_bord_r = if parent_node.border_right.get().unwrap_or(0) > 0 { 1 } else { 0 };
    let p_bord_b = if parent_node.border_bottom.get().unwrap_or(0) > 0 { 1 } else { 0 };
    let p_bord_l = if parent_node.border_left.get().unwrap_or(0) > 0 { 1 } else { 0 };
    ...
    // Flex properties
    let dir = parent_node.flex_direction.get().unwrap_or(0);
    let wrap = parent_node.flex_wrap.get().unwrap_or(WRAP_NOWRAP);
    let justify = parent_node.justify_content.get().unwrap_or(0);
    let align_items = parent_node.align_items.get().unwrap_or(ALIGN_STRETCH);
    let gap = parent_node.gap.get().unwrap_or(0);
    let overflow = Overflow::from(parent_node.overflow.get().unwrap_or(0));
        ...
        let kid_main = INTRINSIC_W.with(|w| {
            INTRINSIC_H.with(|h| {
                ...
                if let Some(kid_node) = get_flex_node(kid) {
                    let ew = resolve_dimension(kid_node.width.get().unwrap_or(Dimension::Auto), content_w);
                    let eh = resolve_dimension(kid_node.height.get().unwrap_or(Dimension::Auto), content_h);
    ...
    for li in 0..line_count {
        ...
        for fi in l_start..=l_end {
            ...
            if let Some(kid_node) = get_flex_node(kid) {
                total_grow += kid_node.flex_grow.get().unwrap_or(0.0);
                total_shrink += kid_node.flex_shrink.get().unwrap_or(1.0);
            } else {
            ...
            let (kid_main, kid_cross) = INTRINSIC_W.with(|w| {
                INTRINSIC_H.with(|h| {
                    ...
                    if let Some(kid_node) = get_flex_node(kid) {
                        let ew = resolve_dimension(kid_node.width.get().unwrap_or(Dimension::Auto), content_w);
                        let eh = resolve_dimension(kid_node.height.get().unwrap_or(Dimension::Auto), content_h);
                        let basis = resolve_dimension(kid_node.flex_basis.get().unwrap_or(Dimension::Auto), if is_row { content_w } else { content_h });
                        ...
                        km = clamp_dimension(
                            km,
                            if is_row { kid_node.min_width.get().unwrap_or(Dimension::Auto) } else { kid_node.min_height.get().unwrap_or(Dimension::Auto) },
                            if is_row { kid_node.max_width.get().unwrap_or(Dimension::Auto) } else { kid_node.max_height.get().unwrap_or(Dimension::Auto) },
                        ...
                        kc = clamp_dimension(
                            kc,
                            if is_row { kid_node.min_height.get().unwrap_or(Dimension::Auto) } else { kid_node.min_width.get().unwrap_or(Dimension::Auto) },
                            if is_row { kid_node.max_height.get().unwrap_or(Dimension::Auto) } else { kid_node.max_width.get().unwrap_or(Dimension::Auto) },
    ...
    for li in 0..line_count {
        ...
        for fi in l_start..=l_end {
            ...
            let (m_top, m_right, m_bottom, m_left) = if let Some(kid_node) = get_flex_node(kid) {
                (
                    kid_node.margin_top.get().unwrap_or(0),
                    kid_node.margin_right.get().unwrap_or(0),
                    kid_node.margin_bottom.get().unwrap_or(0),
                    kid_node.margin_left.get().unwrap_or(0),
                )
            } else {
                (0, 0, 0, 0)
            };
            ...
            let self_align = if let Some(kid_node) = get_flex_node(kid) {
                let a = kid_node.align_self.get().unwrap_or(ALIGN_SELF_AUTO);
...
fn layout_absolute(
    ...
    while let Some(c) = container {
        if indices.contains(&c) {
            if let Some(node) = get_flex_node(c) {
                if node.position.get().unwrap_or(POS_RELATIVE) != POS_RELATIVE {
                    break;
                }
            }
        }
    ...
    // Resolve dimensions
    let ew = resolve_dimension(node.width.get().unwrap_or(Dimension::Auto), container_w);
    let eh = resolve_dimension(node.height.get().unwrap_or(Dimension::Auto), container_h);
    let abs_w = clamp_dimension(
        INTRINSIC_W.with(|w| if ew > 0 { ew } else { w.borrow()[i] }),
        node.min_width.get().unwrap_or(Dimension::Auto),
        node.max_width.get().unwrap_or(Dimension::Auto),
        container_w,
    );
    let abs_h = clamp_dimension(
        INTRINSIC_H.with(|h| if eh > 0 { eh } else { h.borrow()[i] }),
        node.min_height.get().unwrap_or(Dimension::Auto),
        node.max_height.get().unwrap_or(Dimension::Auto),
        container_h,
    );
