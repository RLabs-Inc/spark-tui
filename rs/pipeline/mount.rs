...
use crate::state::{animate, mouse, input, global_keys};
...
pub fn mount() -> io::Result<MountHandle> {
    ...
    let stop: Box<dyn FnOnce()> = match mode {
        RenderMode::Fullscreen => {
            ...
            let mut log_file = std::fs::File::create("/tmp/spark_renders.log").ok();
            ...
            let stop_fn = effect(move || {
                ...
                render_count += 1;
                // Debug: write render count to file
                if let Some(ref mut f) = log_file {
                    use std::io::Write;
                    let _ = writeln!(f, "[RENDER #{}] {:?}", render_count, std::time::Instant::now());
                    let _ = f.flush();
                }
...
/// Run the event loop once with adaptive polling.
///
...
/// # Adaptive Polling
///
/// This function uses adaptive timeout based on animation state:
/// - When animations are active (cursor blink): 16ms (60fps) for smooth rendering
/// - When idle: 250ms to save CPU while staying responsive
///
/// This reduces CPU usage significantly when the application is idle.
///
/// # Arguments
///
...
pub fn tick(handle: &MountHandle) -> io::Result<bool> {
    ...
    // Adaptive timeout: short when animating, longer when idle
    let timeout = if animate::has_active_animations() {
        Duration::from_millis(16) // 60fps for smooth animations
    } else {
        Duration::from_millis(250) // Longer sleep when idle (saves CPU)
    };
...
///
/// Use this for applications that need continuous updates (animations, games,
/// simulations). Just update your signals - the reactive system handles rendering.
///
/// Returns `false` when the app should stop (Ctrl+C or quit).
///
/// # Example
///
/// ```ignore
/// let handle = mount()?;
/// loop {
///     if !poll_input(&handle)? {
///         break;
///     }
///     // Update your signals - reactive system renders automatically
///     particles.update(|ps| { /* physics */ });
/// }
/// unmount(handle);
/// ```
pub fn poll_input(handle: &MountHandle) -> io::Result<bool> {
    if !handle.is_running() {
        return Ok(false);
    }

    // Non-blocking poll - just check if there's input ready
    if let Some(event) = input::poll_event(Duration::ZERO)? {
        input::route_event(event);
    }
...
/// Run the event-driven main loop (zero CPU when idle).
///
/// This is the preferred way to run a TUI application. It uses OS-level
/// event notification and only wakes when:
/// - Terminal input arrives (keyboard, mouse, resize)
/// - Cursor blink timer fires
/// - Stop is requested
///
/// # Example
///
/// ```ignore
/// use spark_tui::pipeline::{mount, events::{EventChannel, InputThread}};
///
/// let handle = mount::mount()?;
/// let channel = EventChannel::new();
/// let _input_thread = InputThread::spawn(channel.sender())?;
///
/// // Zero CPU when idle - truly reactive!
/// mount::run_event_driven(&handle, &channel)?;
/// ```
pub fn run_event_driven(
    handle: &MountHandle,
    channel: &super::events::EventChannel,
) -> io::Result<()> {
    use super::events::{AppEvent, convert_crossterm_event};
    use crate::state::input;

    while handle.is_running() {
        match channel.recv() {
            Some(AppEvent::Input(event)) => {
                // Convert and route the event
                let input_event = convert_crossterm_event(event);
                input::route_event(input_event);
            }
            Some(AppEvent::BlinkTick) => {
                // Blink signal already updated by timer thread.
                // The reactive system will handle re-rendering.
                // Nothing to do here - we just needed to wake up.
            }
            Some(AppEvent::Stop) | None => {
                // Stop requested or channel disconnected
                break;
            }
        }
    }
