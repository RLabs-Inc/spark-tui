...
thread_local! {
    /// Cached parent→children mapping to avoid rebuilding every frame.
    static CHILD_MAP_CACHE: RefCell<Option<HashMap<usize, Vec<usize>>>> = const { RefCell::new(None) };

    /// Dirty flag - set to true when components are added/removed.
    static CHILD_MAP_DIRTY: Cell<bool> = const { Cell::new(true) };

    /// Cached roots list (components with no parent).
    static ROOTS_CACHE: RefCell<Option<Vec<usize>>> = const { RefCell::new(None) };
}

/// Invalidate the child map cache.
///
/// Call this when components are added or removed from the registry.
/// Uses try_with to safely handle thread-local destruction during test cleanup.
pub fn invalidate_child_map() {
    // Use try_with to avoid panicking if TLS is being destroyed
    let _ = CHILD_MAP_DIRTY.try_with(|d| d.set(true));
}

/// Get or build the cached child map.
///
/// Returns a tuple of (child_map, roots) where:
/// - child_map: HashMap<parent_index, Vec<child_indices>> sorted by z-index
/// - roots: Vec<root_indices> sorted by z-index
fn get_or_build_child_map(indices: &[usize]) -> (HashMap<usize, Vec<usize>>, Vec<usize>) {
    let is_dirty = CHILD_MAP_DIRTY.with(|d| d.get());

    if !is_dirty {
        // Try to use cached values
        let cached = CHILD_MAP_CACHE.with(|cache| cache.borrow().clone());
        let cached_roots = ROOTS_CACHE.with(|roots| roots.borrow().clone());

        if let (Some(map), Some(roots)) = (cached, cached_roots) {
            return (map, roots);
        }
    }

    // Build fresh child map
    ...
    // Cache the results
    CHILD_MAP_CACHE.with(|cache| {
        *cache.borrow_mut() = Some(child_map.clone());
    });
    ROOTS_CACHE.with(|roots_cache| {
        *roots_cache.borrow_mut() = Some(roots.clone());
    });
    CHILD_MAP_DIRTY.with(|d| d.set(false));
...
pub fn create_frame_buffer_derived(
    ...
    derived(move || {
        ...
        // Get or build cached child map and roots
        let (child_map, roots) = get_or_build_child_map(&indices);
...
fn render_component(
    ...
    let (pad_top, pad_right, pad_bottom, pad_left, border_t, border_r, border_b, border_l) =
        if let Some(node) = get_flex_node(index) {
            (
                node.padding_top.get().unwrap_or(0),
                node.padding_right.get().unwrap_or(0),
                node.padding_bottom.get().unwrap_or(0),
                node.padding_left.get().unwrap_or(0),
                if node.border_top.get().unwrap_or(0) > 0 { 1u16 } else { 0 },
                if node.border_right.get().unwrap_or(0) > 0 { 1u16 } else { 0 },
                if node.border_bottom.get().unwrap_or(0) > 0 { 1u16 } else { 0 },
                if node.border_left.get().unwrap_or(0) > 0 { 1u16 } else { 0 },
            )
        } else {
            (0, 0, 0, 0, 0, 0, 0, 0)
        };
