...
use super::dirty::INTERACTION_DIRTY_SET;
...
thread_local! {
    ...
    /// Cursor visible flag.
    static CURSOR_VISIBLE: TrackedSlotArray<bool> = tracked_slot_array(
        Some(true),
        INTERACTION_DIRTY_SET.with(|s| s.clone())
    );

    /// Cursor blink FPS (0 = no blink).
    static CURSOR_BLINK_FPS: TrackedSlotArray<u8> = tracked_slot_array(
        Some(2),
        INTERACTION_DIRTY_SET.with(|s| s.clone())
    );

    /// Stick to bottom (auto-scroll on content growth).
    static STICK_TO_BOTTOM: TrackedSlotArray<bool> = tracked_slot_array(
        Some(false),
        INTERACTION_DIRTY_SET.with(|s| s.clone())
    );

    /// Previous max scroll Y (to detect content growth).
    static PREV_MAX_SCROLL_Y: TrackedSlotArray<u16> = tracked_slot_array(
        Some(0),
        INTERACTION_DIRTY_SET.with(|s| s.clone())
    );
    ...
    static CURSOR_CHAR: TrackedSlotArray<u32> = tracked_slot_array(
        Some(0),
        INTERACTION_DIRTY_SET.with(|s| s.clone())
    );
    ...
    static CURSOR_ALT_CHAR: TrackedSlotArray<u32> = tracked_slot_array(
        Some(0),
        INTERACTION_DIRTY_SET.with(|s| s.clone())
    );
    ...
    static CURSOR_STYLE: TrackedSlotArray<u8> = tracked_slot_array(
        Some(0),
        INTERACTION_DIRTY_SET.with(|s| s.clone())
    );
}
...
pub fn get_cursor_visible(index: usize) -> bool {
    CURSOR_VISIBLE.with(|arr| arr.get(index)).unwrap_or(true)
}
...
pub fn get_cursor_blink_fps(index: usize) -> u8 {
    CURSOR_BLINK_FPS.with(|arr| arr.get(index)).unwrap_or(2)
}
...
pub fn get_stick_to_bottom(index: usize) -> bool {
    STICK_TO_BOTTOM.with(|arr| arr.get(index)).unwrap_or(false)
}
...
pub fn get_prev_max_scroll_y(index: usize) -> u16 {
    PREV_MAX_SCROLL_Y.with(|arr| arr.get(index)).unwrap_or(0)
}
...
pub fn get_cursor_char(index: usize) -> u32 {
    CURSOR_CHAR.with(|arr| arr.get(index)).unwrap_or(0)
}
...
pub fn get_cursor_alt_char(index: usize) -> u32 {
    CURSOR_ALT_CHAR.with(|arr| arr.get(index)).unwrap_or(0)
}
...
pub fn get_cursor_style(index: usize) -> u8 {
    CURSOR_STYLE.with(|arr| arr.get(index)).unwrap_or(0)
}
