...
use super::dirty::TEXT_DIRTY_SET;
...
thread_local! {
    /// Text content string.
    static TEXT_CONTENT: TrackedSlotArray<String> = tracked_slot_array(
        Some(String::new()),
        TEXT_DIRTY_SET.with(|s| s.clone())
    );

    /// Text attributes (bold, italic, etc.).
    static TEXT_ATTRS: TrackedSlotArray<Attr> = tracked_slot_array(
        Some(Attr::NONE),
        TEXT_DIRTY_SET.with(|s| s.clone())
    );

    /// Text alignment.
    static TEXT_ALIGN: TrackedSlotArray<TextAlign> = tracked_slot_array(
        Some(TextAlign::Left),
        TEXT_DIRTY_SET.with(|s| s.clone())
    );

    /// Text wrap mode.
    static TEXT_WRAP: TrackedSlotArray<TextWrap> = tracked_slot_array(
        Some(TextWrap::Wrap),
        TEXT_DIRTY_SET.with(|s| s.clone())
    );
}
...
pub fn get_text_content(index: usize) -> String {
    TEXT_CONTENT.with(|arr| arr.get(index)).unwrap_or_default()
}
...
pub fn get_text_align(index: usize) -> TextAlign {
    TEXT_ALIGN.with(|arr| arr.get(index)).unwrap_or(TextAlign::Left)
}
...
pub fn get_text_wrap(index: usize) -> TextWrap {
    TEXT_WRAP.with(|arr| arr.get(index)).unwrap_or(TextWrap::Wrap)
}
